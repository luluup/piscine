...
if(isset($_POST['buttoncc'])) 
{
  //recuperation donnees du formulaire
   ...
   if(!empty($emailclient) AND !empty($mdpclient)) 
   {
      ...
      $dataclientrecup = $bdd->prepare("SELECT * FROM client WHERE email = ? AND motdepasse = ?");
      ...
      if($clienttrouve == 1) 
      {
         ...
         header("Location: affichageclient.php?id=".$_SESSION['id']);

      } 
      else {
         $erreur = "Ce compte client n'existe pas.";
         //On envoie l'user se creer un compte
         header("Location: creationcc.php");
            }
   } 
   else {
...
?>
  <html>
     <head>
        <title>Connexion a mon compte client</title>
        <meta charset="utf-8">
     </head>
     <body>
          <div id="header">
          ...
          <div align="left">
             <h2>Connexion a mon compte client</h2>
             <br/>
